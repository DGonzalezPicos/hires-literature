...
def add_my_copyright(ax, color='black', lower=False, much_lower=False):
    today = date.today()
    ...
    url = 'https://zenodo.org/badge/latestdoi/527634198'
    ax.annotate("Adapted from Arjun Savel et al. 2022 (github.com/arjunsavel/hires-literature).", xy=xy1, xytext=xy1, zorder=100, 
                xycoords='axes fraction', annotation_clip=False, color=color)

    
    # ax.annotate(f"""Generated on {today.strftime("%m/%d/%Y")}""", xy=xy2, xytext=xy2,
    #                 url=url, color=color,
    #                 bbox=dict(color='w', alpha=1e-6, url=url), zorder=100, annotation_clip=False,
    #            xycoords='axes fraction')
    
    ax.annotate(f"""Gif generated by Darío González Picos on {today.strftime("%m/%d/%Y")}\n@github.com/DGonzalezPicos""", xy=xy2, xytext=xy2,
                url=url, color=color,
                bbox=dict(color='w', alpha=1e-6, url=url), zorder=100, annotation_clip=False,
           xycoords='axes fraction')
    return
...
def myplots(observe_frame):
    
    hrs_frame = observe_frame[observe_frame.SNR > 4.]
    ...
    years = np.arange(2012, 2023, 1, dtype=int)[::-1]
    ...
    for i, year in enumerate(years):
        ...
        p = ax.scatter(hrs_frame[hrs_frame.year <= year]['planet'], hrs_frame[hrs_frame.year <= year]['species'], 
                   c=hrs_frame[hrs_frame.year <= year]['SNR'], s=80, cmap=cmap, vmin=4., vmax=18)
    ...
    add_my_copyright(ax, color='black', lower=True)
#     ax.text(-2, -10, f"""Arjun Savel — {datetime.today().strftime('%Y-%m-%d')}""", fontsize=15)


    # fig.savefig('my_plots/year{:}_histogram.png'.format(year), bbox_inches='tight', dpi=300)
    
    
def hrs_obs_latest_gridded(observe_frame, year=2022, highlight=True):
    """
    todo: maybe just hrs_frame once.
    """
    hrs_frame = observe_frame[observe_frame.year > 2009.]
    hrs_frame = hrs_frame[hrs_frame.SNR > 4.]
    hrs_frame = hrs_frame[hrs_frame.SNR > 4.]
    hrs_frame = hrs_frame.sort_values(by=['planet'])
    hrs_frame = hrs_frame.replace({'species': 'C2H2'}, {'species': r'C$_2$H$_2$'}, regex=True)
    hrs_frame = hrs_frame.replace({'species': 'NH3'}, {'species': r'NH$_3$'}, regex=True)
    ...
    # current
    hrs_current = hrs_frame[hrs_frame.year == year]
    if highlight:
        p = ax.scatter(hrs_frame[hrs_frame.year <= year]['planet'], hrs_frame[hrs_frame.year <= year]['species'], 
                       c=hrs_frame[hrs_frame.year <= year]['SNR'], s=80, cmap=cmap, vmin=4., vmax=18, alpha=0.5)
        
        p = ax.scatter(hrs_current['planet'], hrs_current['species'], 
                   c=hrs_current['SNR'], s=140, cmap=cmap, vmin=4., vmax=18)
        
    else: # plot all the previous years with same highlighting as last plot
        p = ax.scatter(hrs_frame[hrs_frame.year <= year]['planet'], hrs_frame[hrs_frame.year <= year]['species'], 
                   c=hrs_frame[hrs_frame.year <= year]['SNR'], s=80, cmap=cmap, vmin=4., vmax=18, alpha=1)
    c = plt.colorbar(p,  pad=0)
    c.set_label(label='SNR', fontsize=25)
    # ax.xticks(rotation = 45);
    ...
    cmap_bar = plt.get_cmap('inferno', 12)
    # rescale = lambda y: (y - np.min(y)) / (np.max(y) - np.min(y))
    
    ind_color = int(year - 2012)
    hrs_frame_year = hrs_frame[hrs_frame.year <= year]
    # years = hrs_frame_year.year.unique().size
    colors = np.array([cmap_bar(x) for x in range(12)])
    # print(years)
    # hist = hrs_frame_year.hist('year', bins=np.arange(2010, 2024), ax=ax2, color='gray', label='Detections',
    #               grid=False, edgecolor='k')
    
    N, bins, patches = ax2.hist(hrs_frame_year.year, bins=np.arange(2010, 2024), label='Detections', edgecolor='k')
    new_det = len(hrs_current)
    if highlight:
        ax2.text(s='+{:}'.format(new_det), x=0.05, y=0.80, fontsize=22, transform=ax2.transAxes)
    else:
        ax2.text(s='Total {:}'.format(len(hrs_frame)), x=0.05, y=0.80, fontsize=22, transform=ax2.transAxes)
    for i in range(len(N)):
        patches[i].set_facecolor(cmap_bar(i))
...
#     ax.text(-2, -10, f"""Arjun Savel — {datetime.today().strftime('%Y-%m-%d')}""", fontsize=15)
    ...
    if highlight is False:
        year += 1
    outfig ='my_plots/hrs_detections_year-{:}.png'.format(year)
...
if __name__=='__main__':
    ...
    # years = np.arange(2010, 2023, 1, dtype=int)

    # for year in years:
    #     hrs_obs_latest_gridded(observe_frame, year=year)
    # hrs_obs_latest_gridded(observe_frame, year=2022, highlight=False) # final plot
    #%%
    create_gif = True
    if create_gif:
        print('Generating animated gif...')
        gif_dir = 'my_plots/'
        os.system('gifski --fps 1 --width 4096 -o {0:}output.gif {0:}{1:}-*.png'.format(gif_dir, 'hrs_detections_year'))   
    # hrs_obs_latest_gridded(observe_frame, year='all')
